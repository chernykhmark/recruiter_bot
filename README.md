...
Сервис автоматически собирает отклики на вакансии с рекрутингового сайта (hh.ru), с помощью DeepSeek через OpenRouter оценивает соответствие каждого кандидата вакансии, отсеивает нерелевантных и присылает рекрутеру в Telegram топ подходящих кандидатов с обоснованием, кому стоит позвонить и почему.
...
└─────────────┘    └──────────────┘    └───────────────┘
      │                   │                    │
 ChromeDriver     OpenRouter/DeepSeek     Telegram
...
│   └── openai_analyzer.py # двухэтапная оценка через OpenRouter/DeepSeek
│
├── notifiers/
...
**Реализация `OpenRouterAnalyzer` — двухэтапная схема (важно для экономии):**

**Этап 1 — быстрый отсев мусора (батчами):**
- Резюме группируются по 5–10 штук в один запрос.
- DeepSeek V3.2 отвечает по каждому: `relevant` / `trash`.
...
- API-ключ OpenRouter, Telegram token + chat_id.
...
**Этап 3 — Анализ**
- `OpenRouterAnalyzer`: сначала этап 2 (детальная оценка), проверить качество.
